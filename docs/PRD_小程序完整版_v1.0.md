...
**应用入口 → 渠道码 → 初始化（新客 onboarding）**

> ⚠️ **已于 2026-07-01(2) 调整**：**登录 / 渠道码「提交」/ 跳过登录 均直接进首页，取消进入流程中的「完善公司信息（初始化）」步骤**；初始化 / 公司维护改由「我的 → 我的公司 → 新增公司」承接。以下 two-entrance → unified-init 为历史设计，仅作背景参考。
>
> **设计确认（two-entrance → unified-init）**：「输入合作伙伴渠道码」有**两个入口**，**两者的「进入 / 跳过」都先经过初始化页，不直接进首页**，保证任何路径都不会漏掉「是否已有香港公司」这一步：
> - **入口 A · 启动页（splash）**：App 启动时的整页渠道入口（首次/未授权态）。
> - **入口 B · 首页 banner → 渠道码页（s-channel）**：在新客首页点击顶部 banner 进入的应用内页，便于已进入用户**补填 / 重填**渠道码。

- 两入口均为：6 位合作伙伴推荐码，**选填、可跳过**（已去除「享专属价格」文案）。
- 点「确认·进入」或「跳过·直接进入」→ **统一进入初始化页**（不直接进首页）：
  - 问「**你是否已有香港公司？**」——「我已有香港公司，去维护」→ **公司基本信息表单**（公司中英名 / 公司编号 CI / 商业登记号 BR / 成立日期 / 注册办事处地址，默认可用法定秘书地址）→ 保存（写入「我的公司」档案）→ 进新客首页。
  - 「暂时没有，先进入首页」或右上「跳过 ›」→ 直接进新客首页。
- 合规：表单注明「仅用于办理与合规提醒，符合 PDPO 个人资料保障」。

```mermaid
flowchart LR
  A[入口A·启动页 splash] --> C[输入合作伙伴渠道码<br/>选填·可跳过]
  B[入口B·首页 banner] --> C
  C -->|确认·进入 / 跳过·直接进入| D[初始化页<br/>是否已有香港公司？]
  D -->|我已有| E[公司基本信息表单<br/>中英名/CI/BR/成立日期/地址]
  E -->|保存·写入我的公司| F[新客首页]
  D -->|暂时没有 / 跳过| F
```
...
| 状态 | 判定 | 进入版本 |
|---|---|---|
| guest 未授权 | 无登录态 | 渠道码入口页 → 初始化页 → 新客版 |
...
- **公司管理（新增 / 删除）**：页头「共 N 家公司」+「**新增公司**」按钮 → 公司基本信息表单（中英名 / CI / BR / 成立日期 / 地址，与初始化页共用）；每张公司卡底部「**删除**」采用**就地二次确认**（确认 / 取消，非系统弹窗，贴合小程序交互）；**有进行中订单的公司禁止删除**——卡片显示「🔒 有进行中订单·暂不可删除」。删除仅移除小程序内档案展示，**不影响政府登记**。
...
**页面内容**：头像/昵称（顶部 KYC 状态标签）、绑定渠道（仅显示"已绑定"，佣金不可见）、我的公司、我的账单、**「KYC认证」入口**、通知订阅、隐私与数据、关于。
