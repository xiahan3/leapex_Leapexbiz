...
- 启动页（splash）或首页 banner → **「输入合作伙伴渠道码」页**：6 位合作伙伴推荐码，**选填、可跳过**（已去除「享专属价格」文案）。
- 点「确认·进入」或「跳过·直接进入」→ 统一进入 **初始化页**（不直接进首页）：
...
- **渠道码页 / 启动页 → 初始化页**：渠道码「确认·进入 / 跳过·直接进入」与启动页 spEnter/spSkip **统一先进初始化页**（不直接进首页）；两处**去除「享专属价格」文案**；渠道码页返回指向新客首页。
