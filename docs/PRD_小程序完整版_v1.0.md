...
- ② 商标查询：商标关键字（英文/繁体）。
- 底部：「剩余 3 次免费」+「提交人工核名」。
- 提交后 →「人工核名中（1–2 天）」状态页（回显方式/名称/商标关键字）→「好的，进入下一步」。
- **结果页**：公司名称查册结果（多条，每条 公司名/BR 号/名称现况/公司现况，可「查看全部」底部浮窗）+ 商标查询结果（商标/编号/Status/Class/拥有人，可「查看全部」浮窗）+ **顾问建议**（一条）+「新公司注册」/「重新核名」。
...
- 服务包含（明细清单）。
- **办理流程**：注册类 = 支付 → 公司注册信息 → KYC 认证；非注册类 = 选择公司 → 支付 → 提交信息 → KYC。
- 办理时效 / 所需材料 / 交付物。
- 富文本详细描述（后台维护）。
...
- 底部弱化「取消订单」文字超链。
- **支付完成页（注册类专属）**：顶部流程 支付→KYC认证→提交注册信息；两豆腐块（① 完成 KYC 实名认证 + AMLO 必要性；② 提交公司注册信息 + 4 步骤：公司资料/成员董事秘书/人工预审/业务办理）。
...
**关键规则（HK 修正）**：平台**不沉淀资金**，宜走**持牌聚合收款**（连连等）；港币用香港主体账户；退款一期人工线下，页面写明"KYC 不通过全额退款"。
...
3. **人工预审**：资料汇总 + NNC1 预览 + 确认勾选 → 提交。
4. **业务办理**：进度时间轴（人工预审通过 → 递交香港公司注册处 → 政府审批 → 签发 CI + BR）。
5. **完结**：注册完成（CI/BR 号、成立日期）+「查看交付物」。
...
**关键规则（HK 修正）**：中/英文名二选一；股份默认普通股；注册地址默认法定秘书地址；**法定秘书须香港居民或持牌 TCSP**（Leapexbiz 担任）；身份字段与 KYC **公共模块**复用，不重复填。
...
10. **退款话术**：一期人工线下退；"KYC 不通过全额退款"须与实际线下流程一致，避免纠纷。
