...
> **文档定位**：覆盖 TCSP 小程序（客户端）**全部已实现功能** + **补全订单 / 消息模块**的完整 PRD；**正文（§一–§九）已同步至最新原型（2026-07-01）**，历史变更留痕见 §十。
> **作者**：财税公司 IT 软件经理（兼香港财务会计）
> **配套**：管理后台 PRD v1.0 + 重构 v3.0、小程序设计规范 v1.0、0625/0626/0630 会议提炼。（本文为**唯一现行小程序 PRD**，已合并历次 TCSP/双版本/完整版等旧版内容。）
...
```mermaid
graph TD
  LG[启动页·服务地区选择 + 渠道码 + 登录] --> A[Leapexbiz 小程序]
  A --> H[首页·双版本]
  A --> S[服务市场]
  A --> O[订单]
  A --> M[消息]
...
| 功能模块 | 功能点 | 优先级 | 核心价值 |
|---|---|---|---|
| **启动 / 登录** | 服务地区选择（港/新/迪拜/BVI）+ 渠道码 + 手机验证码/微信登录 | P0 | 授权入口、地区分流、渠道归属 |
...
flowchart LR
  ...
  B --> C[支付完成: 两条并行待办]
  C --> K[① KYC 认证→人工审核中]
  C --> D[② 提交注册信息: 公司资料→成员/董事/秘书→组织章程→人工预审]
  D --> G[业务办理: 线下递交NNC1·约1周取正本]
  G --> H[完结: CI/BR + 交付物]
```
...
flowchart TD
  ...
  subgraph Reg流程[新公司注册]
    R1[立即办理→支付] --> R2[支付完成·两条并行待办]
    R2 --> RK[① KYC 认证→人工审核中]
    R2 --> R3[② 提交注册信息 NNC1: 公司资料→成员/董事/秘书→组织章程→人工预审]
    R3 --> R6[业务办理: 线下递交NNC1·约1周取正本]
    R6 --> R7[完结: CI/BR]
  end
...
**新客版（咨询接待台）页面内容**
- 信任头图（banner）：Leapexbiz·香港持牌 TCSP·牌照号；价值口号（不裸上身份证）；底部「**有合作伙伴渠道码？点此填写 ›**」——**点击整张 banner 进入「输入合作伙伴渠道码」页**。
...
**应用入口 · 登录 · 渠道码（新客 onboarding）**

- **启动页（splash）**：整页品牌页，含 ①**选择服务地区**（香港 / 新加坡 / 迪拜 / BVI，**默认香港**；非香港区提示"即将开放、当前以香港为例"）；②**合作伙伴渠道码**（6 位，**选填**）；③主按钮**登录** + 「**跳过登录·先逛逛**」文字链。
- **登录页**：**手机号 + 短信验证码**，或 **微信授权登录**；未注册手机号自动创建账户。**登录成功后直接进入首页**（新客版）；「跳过登录·先逛逛」同样直达首页。**不再有「完善公司信息（初始化）」中间步骤**。
- **首页 banner → 渠道码页（s-channel）**：新客首页顶部 banner 可点（「有合作伙伴渠道码？点此填写 ›」）→「输入合作伙伴渠道码」页（6 位·选填）；主按钮「**提交**」、下方「**回到首页**」文字链；**两者均直接回首页**（提交时若填了有效码则绑定该渠道）。
- 公司信息维护改由「**我的 → 我的公司 → 新增公司**」承接（见 §5.10），**不在进入流程强制**。

```mermaid
flowchart LR
  SP[启动页 splash<br/>选择服务地区 + 渠道码选填] -->|登录| LG[登录页<br/>手机验证码 / 微信授权]
  SP -->|跳过登录·先逛逛| HM[新客首页]
  LG --> HM
  HM -->|点顶部 banner| CH[输入合作伙伴渠道码<br/>提交 / 回到首页]
  CH --> HM
```
...
| 状态 | 判定 | 进入版本 |
|---|---|---|
| guest 未授权 | 无登录态 | 启动页（地区+渠道码）→ 登录 → 新客版 |
...
- ① 公司核名查册：查册方式（**以全名查册** / **以名称起首查册**）+ 拟用公司名称**拆两输入**：【**拟用公司全名（繁体中文）**】——框下**实时提示「已帮您转为繁体：XXX」**（内置常用商业字简→繁转换，方便内地客户输简体）；【**拟用公司全名（英文）**】。中 / 英文名**各自查册、互不需对应**（可能英文过、中文不过 → 提示改中文或不要中文名）。
- ② 商标查询：商标关键字（英文/繁体）。
- 底部：「剩余 3 次免费」+「提交人工核名」。
...
- 金额（随服务标准价）+ 状态标签。
- 费用明细（服务名 + 应付合计）。
- **优惠券**：可选券行 → 底部弹窗选券（**不使用 / 新客注册立减券**）；选中后费用明细**加「优惠券抵扣」行、实时改「应付合计」**，抵扣**币种感知**（人民币 −¥180 / 港币 −HK$200）。
...
3. **组织章程（公司章程）**：样本选择 —「**样本 A（简化格式）**」（推荐）/「**样本 B**」（**已去掉"自订英/中"**——自订章程需律师、超出标准服务范围不接）；另选**章程语言：英文 / 繁体中文**（NNC1 与章程语言可不同）。可预览全文。依《公司条例》(Cap. 622)，样本 A/B 为注册处标准章程范本；单一 / 简单股权结构建议**样本 A**。
4. **人工预审**：资料汇总 + NNC1 + 章程预览 + 确认勾选 →「提交人工预审」。
— 提交人工预审后进入办理阶段 —
5. **业务办理**：进度时间轴（人工预审通过 → **顾问线下递交 NNC1 表**至香港公司注册处 → 签发 CI + BR）；文案「**交表后约 1 周**领取 CI / BR 正本」（顾问跑两趟：一趟交表、一趟取正本）；底部「返回首页」+ 右下角「测试下一步」文字超链跳完结页（演示用）。
...
**关键规则（HK 修正）**：中/英文名二选一；股份默认普通股；**组织章程仅样本 A/B（默认 A）+ 语言英/繁**（去自订）；注册地址默认法定秘书地址；**法定秘书须香港居民或持牌 TCSP**（Leapexbiz 担任）；**NNC1 须本人签字**（香港无电子签，大陆客户快递签回；防冒充最好面对面看着签）；身份字段与 KYC **公共模块**复用，不重复填。

### 5.6 KYC 尽职调查

**页面内容**
- 向导：合规说明（AMLO 摘要 + 同意）→ **身份认证**〔证件类型 **香港身份证 / 护照 / 其他证件**（去内地身份证；**护照优先**，注册处认护照签名）+ OCR·人工复核；本人验证用 **手机验证码 / 人脸识别**（原邮箱验证已改）〕→ 地址证明（近 3 个月、显示完整姓名地址）→ 业务性质/资金来源 → **提交审核（提交页不展示 PEP / 制裁筛查）**。
...
**关键规则（HK 修正）**：依 AMLO 须查**实益拥有人（股东）+ 董事**；含 **PEP（含家属）+ 制裁筛查**——但 **PEP / 制裁筛查仅后台内部执行、前端界面不展示**（需另购 KYC 数据服务）；**7 年留存**；**转入客户**（已有公司）同样须 KYC；制裁命中不可向客户泄露（no tipping-off）。
...
- **公司管理（新增 / 删除）**：页头「共 N 家公司」+「**新增公司**」按钮 → 公司基本信息表单（中英名 / CI / BR / 成立日期 / 注册地址；**新增 / 编辑复用同一表单**，即原进入流程移除的「完善公司信息」已收敛至此）；每张公司卡底部「**删除**」采用**就地二次确认**（确认 / 取消，非系统弹窗，贴合小程序交互）；**有进行中订单的公司禁止删除**——卡片显示「🔒 有进行中订单·暂不可删除」。删除仅移除小程序内档案展示，**不影响政府登记**。
...
**页面内容**：头像/昵称（顶部 KYC 状态标签）、绑定渠道（仅显示"已绑定"，佣金不可见）、我的公司、我的账单、**优惠券**、**「KYC认证」入口**、通知订阅、隐私与数据、关于。

- **「KYC认证」入口 → 已认证资料页**（原「KYC 资料」改名为「KYC认证」）：点击进入 KYC **已认证页**，展示绿色「KYC 已认证」横幅（AMLO 尽调通过·通过日·留存 7 年）+ 认证对象（**董事 + 实益拥有人/股东**）+ 风险等级；**「已提交资料」清单**：身份证明（身份证 + 护照）/ 地址证明（近 3 个月）/ 业务性质声明 / 资金来源声明 / 实益拥有人声明 / **PEP·制裁筛查（UN/EU/HKMA 无命中）**，各标「已核验 / 已提交」；底注 AMLO 留存 7 年、不对外披露、证件/地址变更及时更新。
- **优惠券**：券列表分「可用 / 已用·过期」——示例 **免费核名券**（点击**直达核名页**）、**新客注册立减券（−HK$200，满 HK$3,800 可用）**；券卡含面额 / 适用范围 / 有效期。下单支付页可选券抵扣（见 §5.4）；规则「不可提现、不可叠加」。
...
> 说明：**正文 §一–§九 已同步至最新原型设计**（登录入口、核名双名简繁、KYC 手机/人脸、章程语言、优惠券、注册并行流程等均已并入正文）。本节仅保留**历史变更留痕**，供追溯。
...
**v1.0 · 2026-06-30 增补（入口 / 初始化 / 我的公司 / KYC认证）**
- **首页 banner 可点**：新客首页顶部 banner 加「有合作伙伴渠道码？点此填写 ›」，点击整张 banner → 「输入合作伙伴渠道码」页。
- **渠道码页 / 启动页 → 初始化页（two-entrance → unified-init，已与客户确认）**：「输入合作伙伴渠道码」两个入口——**入口 A 启动页（splash）/ 入口 B 首页 banner→渠道码页（s-channel）**——的「确认·进入 / 跳过·直接进入」**统一先进初始化页**（不直接进首页），任何路径都不漏「是否已有香港公司」一步；两处**去除「享专属价格」文案**；渠道码页返回指向新客首页。
- **新建「初始化页」**：选「是否已有香港公司」——有 → 公司基本信息表单（中英名 / CI / BR / 成立日期 / 地址）→ 写入「我的公司」→ 进首页；无 / 跳过 → 直接进首页（PDPO 提示）。
- **我的公司（增删管理）**：改为档案列表渲染 + 「新增公司」；公司卡**就地二次确认删除**；**有进行中订单的公司禁删**（🔒 提示）；删除仅移除小程序展示、不影响政府登记。
- **「KYC 资料」改名「KYC认证」**：点击进入 **KYC 已认证资料页**（绿色已认证横幅 + 认证对象董事/实益拥有人 + 已提交资料清单：身份/地址/业务/资金来源/实益拥有人/PEP·制裁筛查，AMLO 留存 7 年）。

**v1.0 · 2026-07-01 增补（0630 会议落地：核名 / KYC / 章程 / 优惠券 / 登录）**
- **核名提交页**：「拟用公司全名」拆为 **繁体中文 + 英文两个输入**；繁体框下方实时提示「**已帮您转为繁体：XXX**」（内置常用商业字简→繁表，未覆盖字原样保留）。依据：国内客户输简体、注册处以繁体为主，中英文名各查、互不需对应。
- **KYC · 身份认证**：邮箱验证 → **手机验证码 / 人脸识别**（二选一）；证件类型改 **香港身份证 / 护照 / 其他证件**（去内地身份证，护照优先，注册处认护照签名）；**提交环节去掉「PEP / 制裁筛查」**（内部保留、界面不展示）。
- **组织章程**：去掉「自订（英/中）」，仅 **样本 A（推荐）/ 样本 B**；新增 **章程语言：英文 / 繁体中文**（NNC1 与章程语言可不同）。
- **业务办理页**：时效文案改「顾问**线下递交 NNC1**，**交表后约 1 周**领取 CI / BR 正本」（顾问跑两趟：交表 + 取正本）。
- **优惠券**：「我的」新增**优惠券**入口 → 券列表（**免费核名券**点击进核名页、**新客注册立减券**）；**支付页可选券**（底部弹窗选券，抵扣**币种感知**：人民币 −¥180 / 港币 −HK$200，实时改「应付合计」）。
- **首屏（splash）**：新增**地区选择（香港 / 新加坡 / 迪拜 / BVI，默认香港）** + **登录按钮**；渠道码改「选填」。点「登录」→ **登录页**（手机验证码 / 微信授权）；保留「跳过登录·先逛逛」。

**v1.0 · 2026-07-01(2) 微调（入口再简化）**
- **首屏地区标签**：「选择注册地区」→「**选择服务地区**」。
- **登录流程去初始化**：登录成功后**直接进首页**，**取消「完善公司信息（初始化）」中间步骤**（「跳过登录·先逛逛」同样直达首页）。
- **渠道码页（banner→s-channel）**：去掉「跳过后订单归属 Leapexbiz 直营」文案；主按钮「确认·进入」→「**提交**」；「跳过·直接进入」按钮→「**回到首页**」（改文字超链）；**提交 / 回到首页均直接进首页**，不再经初始化页。
- 影响：初始化「完善公司信息」页（s-init）从进入流程移除、暂无入口；公司维护改由「我的公司 → 新增公司」（复用同一公司表单）承接。
