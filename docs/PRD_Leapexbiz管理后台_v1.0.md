...
> **更新**：2026-07-03（新增 **卡券管理**〔04 商品管理下〕与 **消息管理**〔08 系统与权限下〕两模块，承接小程序「优惠券」与「消息中心」；**「核名查册」更名「核名&商标」**：客户提交核名繁体 / 核名英文 / 商标繁体三项，**去掉智能预检**，由服务人员人工给出 3 个核查结果 + 命中清单 + 顾问建议）。
> 2026-07-02（正文已对齐小程序最新设计：登录/地区、渠道码、核名双名简繁、KYC 手机·人脸 + EDD 后台、章程语言、业务办理"线下交表约 1 周"、优惠券、**订单协同（进度/备注/文件）**、**服务流程节点**）。
...
| H | **卡券管理（04 商品管理下，新增）** | 承接小程序「优惠券」：**卡券模板**（免费核名券 / 立减券 / 折扣券）+ **发放**（定向/群发/渠道自动）+ **发放记录与核销统计**；账单侧抵扣已存在，此处补齐**发行与管理** |
| I | **消息管理（08 系统与权限下，新增）** | 承接小程序「消息中心」：**消息模板**（合规通知不可退订 / 流程通知 / 年审提醒 / 营销）+ **年审 6 轮递进提醒规则** + **群发公告** + **发送记录**；多渠道（站内 / 微信服务通知 / 邮件） |
...
| Operations 运营 | 执行层 | 核名&商标、订单进度维护、交付物上传、回单初审 |
...
> **2026-07-02(2) 结构调整**：① 商品管理下「服务管理」更名为**「商品」**（商品目录）；② 新增独立**「服务管理」大类（05，置于供应商前）= 服务执行**：维护服务进度、下载客户资料、上传服务人员文件，含**注册公司**（对齐小程序注册流程节点）与**核名&商标**（核名查册升级 · 由客户管理迁入）；③ **交付物、年审/变更**不再作独立模块（交付物并入服务执行/订单文件区，年审并入客户公司·年审台账）；④ 概览去掉营收趋势 + 业务流程总览卡；⑤ **订单详情改为只读**：进度**联动服务管理**、不可在订单侧修改，仅可**取消订单 + 查看/下载**双方文件。

```mermaid
graph LR
  A[管理后台] --> M1[01 概览·数据驾驶舱]
  A --> M2[02 客户管理]
  A --> M3[03 订单管理]
  A --> M4[04 商品管理]
  A --> M5[05 服务管理·服务执行]
  ...
  M3 --> M3b[账单 / 回单确认]
  M4 --> M4a[商品·基础信息 + 流程节点 + 节点资料]
  M4 --> M4b[卡券·模板 + 发放 + 核销统计]
  M5 --> M5x[注册公司·服务执行]
  M5 --> M5y[核名&商标·人工3结果]
  M6 --> M6a[供应商管理]
  M6 --> M6b[订单分配]
  ...
  M8 --> M8c[合规与审计]
  M8 --> M8d[系统参数]
  M8 --> M8e[消息·模板 + 年审提醒 + 群发 + 记录]
```

> **服务执行（05）核心**：由服务节点实例化的订单，在此**维护进度（可编辑）+ 下载客户上传资料 + 上传服务人员文件**；进度回推小程序「办理进度」并**驱动订单状态**。订单管理（03）侧只读展示、不改进度。
...
**③ KYC / EDD 审核管理**
- 队列：待审核（提交时间 + SLA 倒计时）。
- 审核台：小程序提交内容（认证对象/身分、证件类型+OCR、本人验证 手机/人脸、地址证明、业务性质/资金来源）**均可编辑修改**，证件/地址证明支持**下载 + 重新上传**；**PEP / 制裁筛查为后台内部执行、不在审核界面展示**（命中→冻结见状态机）。
- 动作（按状态自适应）：**保存修改 / 通过 / 拒绝 / 标记 EDD**；**已去掉"要求补充"**；点「拒绝」**下方弹出拒绝原因输入框**（必填，通知客户 + 留档 7 年）；EDD → EDD 通过；制裁冻结 → 上报 STR / 解冻放行；已通过·已拒绝为只读。
...
> 小程序现状：`我的 → 优惠券` 展示可用/已用券；`账单页` 支持选券抵扣（券种 + 抵扣额，币种感知）。当前券为**演示数据**，本模块补齐**发行、发放与核销管理**。

- **① 卡券模板**（可编辑，一处配置驱动小程序券面与账单抵扣规则）：

  | 字段 | 说明 |
  |---|---|
  | 券名 | 如"免费核名券""新客注册立减券" |
  | 类型 | **免费服务券**（兑换指定免费服务，如核名）/ **立减券**（固定金额，如 −HK$200）/ **折扣券**（百分比，预留） |
  | 面值 / 币种 | 立减额（双币）或折扣率；免费服务券关联**服务 ID** |
  | 适用范围 scope | 适用服务 / 分类（如"标准·高级注册"）+ **满减门槛**（如满 HK$3,800） |
  | 有效期规则 | **固定截止日**（如 2026-09-30）或**领取后 N 天** |
  | 发放上限 / 每人限领 | 总量与单用户上限 |
  | 叠加 | **不可叠加**（默认，与小程序"不可叠加/不可提现"一致） |
  | 状态 | 上架 / 下架 |

- **② 发放**：
  - **定向发放**（选客户 / 客户群：如"新注册未下单""高价值"）；
  - **群发**（按服务地区 / 渠道）；
  - **渠道自动发放**（如渠道码新客自动获"注册立减券"）；
  - 发放即生成 `COUPON` 实例（归属客户）→ 小程序"我的·优惠券"可见。
- **③ 发放记录与核销统计**：券实例列表（客户 / 券种 / 状态 可用·已用·过期 / 领取时间 / 使用订单 / 使用时间）；模板维度统计**发放数 / 核销数 / 核销率 / 带动 GMV**。
- **规则**：券**不可提现、不可叠加**；核销发生在**账单确认抵扣**（写 `bill.coupon_discount` + `coupon.status=已用`）；过期由系统置 `过期`。
...
> 小程序现状：`消息中心` 分三类展示——**合规通知·不可关闭**（KYC 结果 / 核名结果）、**流程通知**（账单生成 / 补料提醒）、**年审提醒·6 轮递进**；`我的·通知订阅`（合规通知不可关闭）。本模块负责**模板、触发规则、群发与发送记录**。

- **① 消息模板**（按分类维护）：

  | 字段 | 说明 |
  |---|---|
  | 分类 | **合规通知**（强制·不可退订）/ **流程通知** / **年审提醒** / **营销通知**（可退订） |
  | 触发事件 | KYC 通过·拒绝 / 核名结果 / 账单生成 / 待补资料 / 交付完成 / 年审到期 / 卡券发放 / 手动公告 |
  | 标题 / 内容 | 支持**变量占位**（如 `{公司名}`、`{金额}`、`{到期日}`、`{订单号}`） |
  | 渠道 | **站内消息** + **微信服务通知**（模板消息）+ **邮件**（可多选；海外客户以邮件为主） |
  | 强制 | 合规通知**强制下发、不可退订**；营销可退订 |
  | 状态 | 启用 / 停用 |

- **② 年审提醒规则（6 轮递进）**：到期前 **60 / 42 / 30 / 14 / 7 / 1 天** 六个触发点（对齐"成立周年 + 42 天"），逐轮绑定模板 + 渠道 + 开关；命中即向该公司关联客户发年审提醒 + 生成待办。
- **③ 群发 / 公告**：撰写标题 + 内容 → 选**目标人群**（全部 / 按服务地区 / 按公司状态〔年审待办等〕/ 按渠道 / 指定客户）→ 选渠道 → **发送 / 定时发送**；合规类不受退订影响。
- **④ 发送记录**：按接收人 / 分类 / 触发事件 / 渠道 / 状态（已发送·已读·失败·已退订拦截）/ 时间 检索；失败可重发。
- **触发来源**：KYC / 核名 / 账单 / 订单节点 / 年审台账 / 卡券发放等事件**自动触发**模板；运营亦可**手动群发**。与「合规与审计」联动（合规通知留痕）。
...
**COUPON_TEMPLATE 卡券模板（★新增）**：`id · name · type(free_service免费服务券/cash立减券/percent折扣券) · value(立减额或折扣率) · currency · free_service_id(免费服务券关联服务,可空) · scope(适用服务/分类) · min_amount(满减门槛) · valid_type(fixed固定截止/days领取后N天) · valid_until · valid_days · issue_limit(发放上限) · per_user_limit(每人限领) · stackable(默认false) · status(上架/下架) · created_at`
...
**MESSAGE_TEMPLATE 消息模板（★新增）**：`id · category(合规/流程/年审/营销) · trigger_event(kyc_result/nc_result/bill_created/doc_required/delivered/annual_reminder/coupon_issued/announce) · title · body(含变量占位) · channels[](站内/微信服务通知/邮件) · mandatory(强制·不可退订) · annual_round(年审第N轮:60/42/30/14/7/1天,可空) · status(启用/停用)`

**MESSAGE 消息实例 / 发送记录（★新增）**：`id · template_id(可空:手动公告) · customer_id · category · title · body · channel · ref_type(order/company/bill/coupon,可空) · ref_id · status(已发送/已读/失败/已退订拦截) · sent_by(系统触发/员工id) · sent_at · read_at`

**NAME_CHECK 核名&商标申请（2026-07-03 改）**：客户提交三项 `name_zh(核名·繁体) · name_en(核名·英文) · tm_zh(商标·繁体/关键字)` + `mode(全名/起首)`；**无智能预检**，服务人员人工查册后逐项录入 `result_zh{结论,命中公司[]}` · `result_en{结论,命中公司[]}` · `result_tm{结论,命中商标[]}` + `advisor_note`；`status(待核查/核查中/已出结果/待客户确认/名称锁定/已拒绝)`。核名结论=🟢可用/🟡有近似/🟠近似冲突/🔴敏感词·商标冲突/🔵需牌照；商标结论=🟢可注册/🟠有近似/🔴已注册冲突。命中公司={公司名, BR号, 名称现况, 状态(仍注册/已告解散)}；命中商标={商标名, 编号, 类别Class, 拥有人, 状态}。发布后回推小程序「核名 & 商标查询结果」页。`free_quota_used · created_at`
...
- **卡券发放 → 核销**：`coupon_template` 发放 → 生成 `coupon(status=可用)` 归客户（小程序"我的·优惠券"可见）；账单确认抵扣 → `bill.coupon_discount` + `coupon.status=已用/used_order_id`；到期系统置 `过期`；**不可叠加、不可提现**。
- **消息触发 → 下发**：业务事件（KYC/核名/账单/订单节点/年审/卡券）命中 `message_template.trigger_event` → 渲染变量生成 `message` 按 `channels` 下发（站内/微信/邮件）；**合规类强制、忽略退订**；年审按 6 轮 `annual_round` 递进；运营可**手动群发**（`template_id` 空、按目标人群批量生成 `message`）。
...
| 提交核名&商标（核名繁体 + 核名英文 + 商标繁体） | 服务管理·**核名&商标** | NAME_CHECK（无智能预检 · 人工 3 结果） | 3 项人工核查结果 + 命中清单 + 顾问建议 |
...
| 我的·优惠券 / 账单选券 | 商品·**卡券管理** | COUPON_TEMPLATE 发放 → COUPON 实例 → 账单核销 | 券面 + 可用/已用/过期 + 抵扣额 |
| 消息中心 / 通知订阅 | 系统·**消息管理** | MESSAGE_TEMPLATE 触发 → MESSAGE 下发 | 合规/流程/年审/营销 分类消息 |
...
| P1 | 供应商（管理/分配/结算/工作台）· 渠道商（弹窗配置/佣金结算）· 核名审核 · 年审台账 · **卡券管理（模板/发放/核销）** · **消息管理（模板/年审 6 轮/群发/记录）** · 审计日志 |
