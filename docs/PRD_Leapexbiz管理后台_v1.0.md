...
> **更新**：2026-07-03（新增 **卡券管理**〔04 商品管理下〕与 **消息管理**〔08 系统与权限下〕两模块，承接小程序「优惠券」与「消息中心」）。
...
> **2026-07-02(2) 结构调整**：① 商品管理下「服务管理」更名为**「商品」**（商品目录）；② 新增独立**「服务管理」大类（05，置于供应商前）= 服务执行**：维护服务进度、下载客户资料、上传服务人员文件，含**注册公司**（对齐小程序注册流程节点）与**核名查册**（由客户管理迁入）；③ **交付物、年审/变更**不再作独立模块（交付物并入服务执行/订单文件区，年审并入客户公司·年审台账）；④ 概览去掉营收趋势 + 业务流程总览卡；⑤ **订单详情改为只读**：进度**联动服务管理**、不可在订单侧修改，仅可**取消订单 + 查看/下载**双方文件。
